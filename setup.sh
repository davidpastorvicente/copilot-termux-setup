...
# The last known fast Android-native setup path before upstream added runtime.node.
DEFAULT_COPILOT_VERSION="1.0.45"
COPILOT_VERSION="${COPILOT_VERSION:-$DEFAULT_COPILOT_VERSION}"
...
write_copilot_launcher() {
  local launcher="${PREFIX}/bin/copilot"
  rm -f "$launcher"
  cat > "$launcher" <<EOF
#!/data/data/com.termux/files/usr/bin/bash
set -e
exec "${PREFIX}/bin/node" "${INSTALL_ROOT}/app.js" "\$@"
EOF
  chmod +x "$launcher"
}
...
echo "Copilot CLI version target: $COPILOT_VERSION"
echo ""
...
print_info "Running npm install -g @github/copilot@$COPILOT_VERSION"
...
print_step "Step 10/12: Writing Copilot launcher"

write_copilot_launcher
print_success "copilot launcher installed"

print_step "Step 11/12: Symlinking compiled binaries to prebuilds directory"
...
print_step "Step 12/12: Verifying installation"
...
if "${PREFIX}/bin/copilot" --version >/dev/null; then
    print_success "copilot launcher works"
else
    print_error "copilot launcher failed"
    exit 1
fi
...
echo "  • Replaced copilot launcher with direct app.js wrapper"
